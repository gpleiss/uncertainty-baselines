...
def get_data_dir(data_dir: str, dataset_name: str, try_gcs: bool) -> str:
  return data_dir
  ...
  def num_examples(self):
    if isinstance(self._split, tfds.core.ReadInstruction):
      absolute_split = self._split.to_absolute(
          {
              name: self.info.splits[name].num_examples
              for name in self.info.splits.keys()
          })[0]
      return _absolute_split_len(absolute_split, self.info.splits)
    return self.info.splits[self._split].num_examples
